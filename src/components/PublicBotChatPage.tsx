...
export const PublicBotChatPage = () => {
  ...
  // disabled: (bot?.require_visitor_auth0_identity || forceVisitorAuth) && botId
  ...
  return (
    <div className="min-h-screen bg-background flex flex-col items-center justify-center p-4">
      <Card className={`w-full ${bot.is_video_bot ? (showVideoAvatar ? 'max-w-6xl' : 'max-w-2xl') : 'max-w-2xl'} h-[600px] flex flex-col bg-background/95 backdrop-blur-sm border shadow-2xl rounded-xl overflow-hidden transition-all duration-300`}>
        {/* Fixed Header — modern, info-rich (mirrors ChatBot.tsx) */}
        <CardHeader className="relative flex-shrink-0 space-y-0 p-0 overflow-hidden sticky top-0 z-10">
          ...
          <div className="relative px-5 py-4">
            <div className="flex items-start justify-between gap-3">
              <div className="flex items-start gap-3 flex-1 min-w-0">
                ...
                <div className="flex-1 min-w-0">
                  <CardTitle className="text-xl text-white font-semibold tracking-tight truncate" title={bot.name}>
                    {bot.name}
                  </CardTitle>
                  {bot.description && (
            ...
            {/* All info tags (mirrors BotCard / ChatBot.tsx) */}
            <div className="mt-3 flex flex-wrap gap-1.5">
              ...
              <Badge className="text-[11px] bg-white/15 text-white border border-white/25 hover:bg-white/25 backdrop-blur-sm">
                {bot.is_video_bot ? <><Video className="h-3 w-3 mr-1" />Video Bot</> : <><Bot className="h-3 w-3 mr-1" />Chat Bot</>}
              </Badge>
              <Badge className="text-[11px] bg-white/15 text-white border border-white/25 hover:bg-white/25 backdrop-blur-sm">
                {(bot.is_video_bot || bot.is_voice_enabled) ? <><Volume2 className="h-3 w-3 mr-1" />Voice Enabled</> : <><VolumeX className="h-3 w-3 mr-1" />Voice Disabled</>}
              </Badge>
              ...
              {(bot.primary_purpose || bot.primaryPurpose) && (
                <Badge className="text-[11px] bg-white/15 text-white border border-white/25 hover:bg-white/25 backdrop-blur-sm">🎯 {bot.primary_purpose || bot.primaryPurpose}</Badge>
              )}
              {(bot.conversational_tone || bot.conversationalTone) && (
                <Badge className="text-[11px] bg-white/15 text-white border border-white/25 hover:bg-white/25 backdrop-blur-sm">🎭 {bot.conversational_tone || bot.conversationalTone}</Badge>
              )}
              {(bot.target_audience || bot.targetAudience) && (
                <Badge className="text-[11px] bg-white/15 text-white border border-white/25 hover:bg-white/25 backdrop-blur-sm">👥 {bot.target_audience || bot.targetAudience}</Badge>
              )}
              {(bot.conversational_style || bot.conversationalStyle || bot.response_style || bot.responseStyle) && (
                <Badge className="text-[11px] bg-white/15 text-white border border-white/25 hover:bg-white/25 backdrop-blur-sm">🧭 {bot.conversational_style || bot.conversationalStyle || bot.response_style || bot.responseStyle}</Badge>
              )}
              ...
              {(bot.website_url || bot.websiteUrl) && (
              ...
              <Badge className="text-[11px] bg-white/15 text-white border border-white/25 hover:bg-white/25 backdrop-blur-sm">
                🤖 {(bot.custom_llm_provider || bot.customLLMProvider) ? `Custom LLM (${bot.custom_llm_provider || bot.customLLMProvider})` : "Platform LLM"}
              </Badge>
              {(bot.is_slack_enabled || bot.isSlackEnabled) && (
                <Badge className="text-[11px] bg-white/15 text-white border border-white/25 hover:bg-white/25 backdrop-blur-sm">💬 Slack</Badge>
              )}
              {(bot.human_handoff_enabled || bot.humanHandoffEnabled) && (
